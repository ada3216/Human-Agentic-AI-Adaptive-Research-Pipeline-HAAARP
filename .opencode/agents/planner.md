...
COMPOSITE REMEDIATION RULE:
- If the prompt explicitly says this is a plan-review remediation pass, revise `.ai-layer/current-plan.md` in place to address only the listed review items unless the prompt explicitly asks for a replacement approach.
...
7. Enter planning state.
    ...
    - Run: `bash scripts/state.sh set current_task "[task name]"`
...
9. Produce `.ai-layer/current-plan.md`.
    ...
    - ARCHITECTURE north_star: if populated, truth-check each step against it.
      Rewrite misaligned steps where possible. If a material tradeoff remains,
      surface it in `North star alignment` with options, payoff, and pros/cons.
   ...
   - both sets
    ...
    If a step materially conflicts with a populated north_star:
    - prefer the aligned path by default
    - if multiple valid paths remain, document the tradeoff explicitly so
      `/review-plan` can approve the recommended option or choose another
...
## WORKFLOW-END COMMIT (mandatory if the planner writes governed files directly)

Before returning control, run this sequence:
1. `bash scripts/check.sh` — must exit 0 before proceeding
2. Stage only files this command wrote or modified — use `git add <file>...` for each governed file touched. Do NOT use `git add -A`. Unrelated dirty files in the working tree are not this command's responsibility.
...
5. `git status --porcelain` — check for uncommitted files. If any listed files were written by this command, they must be committed (stop and fix). Unrelated dirty files are advisory — report them but do not block.
...
Required for PRODUCT or MIXED tasks when `north_star` is populated.
If all planned steps align: write `All steps aligned with north star: [north_star]`.
If any step has tension:
- Step: [step number or area]
- Tension: [how it conflicts or creates drift risk]
- Option A: [more aligned path] | payoff: [benefit] | pros: [brief] | cons: [brief]
- Option B: [less aligned or alternate path] | payoff: [benefit] | pros: [brief] | cons: [brief]
- Recommendation: [preferred option and why]
If the choice should be made in plan review, state: `Choice needed in /review-plan`.
Notes
...
─────────────────────────────────────────
NEXT STEP
Command:  /review-plan-fix-implement
Model:    Switch to a DIFFERENT AI provider before running /review-plan-fix-implement
Action:   Open a new session on a different provider, then run /review-plan-fix-implement
─────────────────────────────────────────
